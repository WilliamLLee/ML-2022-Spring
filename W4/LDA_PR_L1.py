...
class LDA_PR:
    ...
    def fit(self,X,y,lr = 0.01,max_epoch= 1000,batch_size = 100,epsilon = 1e-6,lambda_l1=0.1,show_parameter= False):
        '''
        ...
        for epoch in range(1,max_epoch+1):
            ...
            for iter in range(0,x_N,batch_size):
                ...
                loss += - np.sum(np.sum(y_batch*np.log(prob)+(1-y_batch)*np.log(1-prob),axis=1)+np.sum(np.abs(self._w),axis=0))
                # update the parameters
                self._w -= lr*(np.dot(X_batch.T,prob-y_batch)+ lambda_l1*np.sign(self._w))
                self._b -= lr*(np.sum(prob-y_batch,axis=0))
            loss = loss/x_N
